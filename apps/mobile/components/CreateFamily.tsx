...
import { Adapt, Button, Dialog, Form, Heading, Input, Sheet, Text, XStack, YStack } from "tamagui"
...
export const CreateFamily = () => {
	...
	const {
		...
		isSuccess,
		isError,
	} = api.family.create.useMutation({
		onSuccess: (data) => {
			console.log(data)
			utils.user.get
				.invalidate()
				.catch(() => toast.show("Some error occured while refetching user"))
		},
	})
	...
	return (
		...
		>
			<Dialog.Trigger asChild>
				<Button backgroundColor="$color.btn-primary" borderRadius="$12" size="$5" circular>
					<Plus size={24} color="$color.text-secondary" />
				</Button>
			</Dialog.Trigger>
			<Adapt when="sm" platform="touch">
				<Sheet snapPointsMode="fit" animation="medium" zIndex={200000} modal dismissOnSnapToBottom>
					<Sheet.Frame padding="$4" paddingBottom="$8" gap="$4" bg="$color.bg-primary">
